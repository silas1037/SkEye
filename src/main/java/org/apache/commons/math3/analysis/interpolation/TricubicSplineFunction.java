...
class TricubicSplineFunction implements TrivariateFunction {
    ...
    /* renamed from: N */
    private static final short f133N = 4;

    /* renamed from: a */
    private final double[][][] f134a = ((double[][][]) Array.newInstance(Double.TYPE, 4, 4, 4));

    TricubicSplineFunction(double[] aV) {
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                for (int k = 0; k < 4; k++) {
                    this.f134a[i][j][k] = aV[(((k * 4) + j) * 4) + i];
                }
            }
        }
    }
    ...
    public double value(double x, double y, double z) throws OutOfRangeException {
        if (x < 0.0d || x > 1.0d) {
            ...
            for (int i = 0; i < 4; i++) {
                for (int j = 0; j < 4; j++) {
                    for (int k = 0; k < 4; k++) {
                        result += this.f134a[i][j][k] * pX[i] * pY[j] * pZ[k];
                    }
                }
            }
            return result;
